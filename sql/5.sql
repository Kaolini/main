...
--				, coalesce(rdo.source_flag, rdf.source_flag) as source_flag
	...
	, problem_list as (
		select
			row_number() over (order by ord, oms, new_date_referral) as num
			, new_date_referral
			, new_district
			, new_mo
			, fio
		...
		from (
			...
			from (
				...
				from pacients p
				where pacient_type in (5,6,8,9) and new_agree = false and old_agree = true and position(new_mkb10 in old_mkb10) > 0
				union all
